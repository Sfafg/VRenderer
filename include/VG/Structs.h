...
struct DeviceFeatures {
    ...
    DeviceFeatures()
        : robustBufferAccess(false), fullDrawIndexUint32(false), imageCubeArray(false), independentBlend(false),
          geometryShader(false), tessellationShader(false), sampleRateShading(false), dualSrcBlend(false),
          logicOp(false), multiDrawIndirect(false), drawIndirectFirstInstance(false), depthClamp(false),
          depthBiasClamp(false), fillModeNonSolid(false), depthBounds(false), wideLines(false), largePoints(false),
          alphaToOne(false), multiViewport(false), samplerAnisotropy(false), textureCompressionETC2(false),
          textureCompressionASTC_LDR(false), textureCompressionBC(false), occlusionQueryPrecise(false),
          pipelineStatisticsQuery(false), vertexPipelineStoresAndAtomics(false), fragmentStoresAndAtomics(false),
          shaderTessellationAndGeometryPointSize(false), shaderImageGatherExtended(false),
          shaderStorageImageExtendedFormats(false), shaderStorageImageMultisample(false),
          shaderStorageImageReadWithoutFormat(false), shaderStorageImageWriteWithoutFormat(false),
          shaderUniformBufferArrayDynamicIndexing(false), shaderSampledImageArrayDynamicIndexing(false),
          shaderStorageBufferArrayDynamicIndexing(false), shaderStorageImageArrayDynamicIndexing(false),
          shaderClipDistance(false), shaderCullDistance(false), shaderFloat64(false), shaderInt64(false),
          shaderInt16(false), shaderResourceResidency(false), shaderResourceMinLod(false), sparseBinding(false),
          sparseResidencyBuffer(false), sparseResidencyImage2D(false), sparseResidencyImage3D(false),
          sparseResidency2Samples(false), sparseResidency4Samples(false), sparseResidency8Samples(false),
          sparseResidency16Samples(false), sparseResidencyAliased(false), variableMultisampleRate(false),
    ...
    DeviceFeatures(Flags<Feature> features)
        : robustBufferAccess(features.IsSet(Feature::RobustBufferAccess)),
          ...
          dualSrcBlend(features.IsSet(Feature::DualSrcBlend)), logicOp(features.IsSet(Feature::LogicOp)),
          ...
          depthClamp(features.IsSet(Feature::DepthClamp)), depthBiasClamp(features.IsSet(Feature::DepthBiasClamp)),
          fillModeNonSolid(features.IsSet(Feature::FillModeNonSolid)),
          depthBounds(features.IsSet(Feature::DepthBounds)), wideLines(features.IsSet(Feature::WideLines)),
          largePoints(features.IsSet(Feature::LargePoints)), alphaToOne(features.IsSet(Feature::AlphaToOne)),
          ...
          shaderFloat64(features.IsSet(Feature::ShaderFloat64)), shaderInt64(features.IsSet(Feature::ShaderInt64)),
    ...
    bool operator==(const DeviceFeatures &rhs) const {
        return robustBufferAccess == rhs.robustBufferAccess && fullDrawIndexUint32 == rhs.fullDrawIndexUint32 &&
               ...
               depthBounds == rhs.depthBounds && wideLines == rhs.wideLines && largePoints == rhs.largePoints &&
               alphaToOne == rhs.alphaToOne && multiViewport == rhs.multiViewport &&
               samplerAnisotropy == rhs.samplerAnisotropy && textureCompressionETC2 == rhs.textureCompressionETC2 &&
               textureCompressionASTC_LDR == rhs.textureCompressionASTC_LDR &&
               ...
               shaderFloat64 == rhs.shaderFloat64 && shaderInt64 == rhs.shaderInt64 && shaderInt16 == rhs.shaderInt16 &&
               shaderResourceResidency == rhs.shaderResourceResidency &&
...
struct ColorBlend {
    ...
    ColorBlend()
        : blendEnable(false), srcColorBlendFactor(BlendFactor::Zero), dstColorBlendFactor(BlendFactor::Zero),
          colorBlendOp(BlendOp::Add), srcAlphaBlendFactor(BlendFactor::Zero), dstAlphaBlendFactor(BlendFactor::Zero),
          alphaBlendOp(BlendOp::Add), colorWriteMask(ColorComponent::RGBA) {}
    ...
    ColorBlend(
        ...
        : blendEnable(true), srcColorBlendFactor(srcColorBlendFactor), dstColorBlendFactor(dstColorBlendFactor),
          colorBlendOp(colorBlendOp), srcAlphaBlendFactor(srcAlphaBlendFactor),
          dstAlphaBlendFactor(dstAlphaBlendFactor), alphaBlendOp(alphaBlendOp), colorWriteMask(colorWriteMask) {}
...
struct SubpassDependency {
    ...
    SubpassDependency(
        uint32_t srcSubpass, uint32_t dstSubpass, Flags<PipelineStage> srcStageMask, Flags<PipelineStage> dstStageMask,
        Flags<Access> srcAccessMask, Flags<Access> dstAccessMask, Flags<Dependency> dependencyFlags
    )
...
struct Attachment {
  ...
  public:
    ...
    Attachment()
        : format(Format::Undefined), samples(1), loadOp(LoadOp::DontCare), storeOp(StoreOp::DontCare),
          stencilLoadOp(LoadOp::DontCare), stencilStoreOp(StoreOp::DontCare), initialLayout(ImageLayout::Undefined),
          finalLayout(ImageLayout::Undefined) {}

    Attachment(
        Format format, ImageLayout finalLayout, ImageLayout initialLayout = ImageLayout::Undefined,
        LoadOp loadOp = LoadOp::Clear, StoreOp storeOp = StoreOp::Store, LoadOp stencilLoadOp = LoadOp::DontCare,
        StoreOp stencilStoreOp = StoreOp::DontCare, unsigned int samples = 1
    )
    ...
    Attachment(
        Format format, unsigned int samples, ImageLayout finalLayout,
        ImageLayout initialLayout = ImageLayout::Undefined, LoadOp loadOp = LoadOp::Clear,
        StoreOp storeOp = StoreOp::Store, LoadOp stencilLoadOp = LoadOp::DontCare,
...
struct VertexBinding {
    ...
    VertexBinding(int binding = 0, int stride = 0, InputRate inputRate = InputRate::Vertex)
...
struct VertexAttribute {
    ...
    VertexAttribute(uint32_t location = 0, uint32_t binding = 0, Format format = Format::Undefined, uint32_t offset = 0)
...
struct VertexLayout {
  ...
  public:
    ...
    VertexLayout(
        const std::vector<VertexBinding> &vertexDescriptions, const std::vector<VertexAttribute> &vertexAttributes
    )
        : vertexDescriptionCount(vertexDescriptions.size()),
          vertexDescritpions(new VertexBinding[vertexDescriptionCount]), vertexAttributesCount(vertexAttributes.size()),
          vertexAttributes(new VertexAttribute[vertexAttributesCount]) {
        memcpy(
            (void *)this->vertexDescritpions, &vertexDescriptions[0], sizeof(VertexBinding) * vertexDescriptionCount
        );
        memcpy((void *)this->vertexAttributes, &vertexAttributes[0], sizeof(VertexAttribute) * vertexAttributesCount);
    }

    VertexLayout(VertexLayout &&rhs) noexcept : VertexLayout() { *this = std::move(rhs); }
...
struct InputAssembly {
  ...
  public:
    ...
    InputAssembly(Primitive primitive, bool primitiveRestart = false)
        : primitive(primitive), primitiveRestart(primitiveRestart) {}
...
struct ViewportState {
  ...
  public:
    ...
    ViewportState(ViewportState &&rhs) : ViewportState() { *this = std::move(rhs); }
...
struct alignas(float) DepthBias {
    ...
    DepthBias(bool enable = false, float constantFactor = 0, float clamp = 0, float slopeFactor = 0)
        : enable(enable), constantFactor(constantFactor), clamp(clamp), slopeFactor(slopeFactor) {}
};
struct Rasterizer {
  private:
  ...
  public:
    ...
    Rasterizer(
        bool depthClamp, bool discard, PolygonMode polygonMode, CullMode cullMode = CullMode::Back,
        FrontFace frontFace = FrontFace::CounterClockwise, DepthBias depthBias = DepthBias(), float lineWidth = 1.0f
    )
        : discard(discard), depthClamp(depthClamp), polygonMode(polygonMode), cullMode(cullMode), frontFace(frontFace),
          depthBias(depthBias), lineWidth(lineWidth) {}
...
struct Multisampling {
  ...
  public:
    ...
    Multisampling(
        uint32_t samples, bool enableSampleShading = false, float minSampleShading = 0, void *sampleMask = nullptr,
        bool enableAlphaToCoverage = false, bool enableAlphaToOne = false
    )
        : rasterizationSamples(samples), sampleShadingEnable(enableSampleShading), minSampleShading(minSampleShading),
          sampleMask(sampleMask), alphaToCoverageEnable(enableAlphaToCoverage), alphaToOneEnable(enableAlphaToOne) {}
...
struct StencilOpState {
    ...
    StencilOpState(
        StencilOp failOp, StencilOp passOp, StencilOp depthFailOp, CompareOp compareOp, uint32_t compareMask,
        uint32_t writeMask, uint32_t reference
    )
        : failOp(failOp), passOp(passOp), depthFailOp(depthFailOp), compareOp(compareOp), compareMask(compareMask),
          writeMask(writeMask), reference(reference) {}
};
...
struct DepthStencil {
  ...
  public:
    DepthStencil() {}
    DepthStencil(bool enableDepthTest, bool enableDepthWrite, CompareOp depthCompareOp)
        : depthTestEnable(enableDepthTest), depthWriteEnable(enableDepthWrite), depthCompareOp(depthCompareOp) {}
    DepthStencil(
        bool enableDepthTest, bool enableDepthWrite, CompareOp depthCompareOp, bool enabledepthBoundsTest,
        float minDepthBounds, float maxDepthBounds
    )
        : depthTestEnable(enableDepthTest), depthWriteEnable(enableDepthWrite), depthCompareOp(depthCompareOp),
          depthBoundsTestEnable(enabledepthBoundsTest), minDepthBounds(minDepthBounds), maxDepthBounds(maxDepthBounds) {
    }
    DepthStencil(
        bool enableDepthTest, bool enableDepthWrite, CompareOp depthCompareOp, StencilOpState front,
        StencilOpState back, bool enabledepthBoundsTest, float minDepthBounds, float maxDepthBounds
    )
        : depthTestEnable(enableDepthTest), depthWriteEnable(enableDepthWrite), depthCompareOp(depthCompareOp),
          depthBoundsTestEnable(enabledepthBoundsTest), stencilTestEnable(true), front(front), back(back),
...
struct ColorBlending {
  ...
  public:
    ...
    ColorBlending(
        bool enableLogicOp, LogicOp logicOp, const float (&blendConstants)[4],
        const std::vector<ColorBlend> &colorBlending = {}
    )
        : enableLogicOp(enableLogicOp), logicOp(logicOp),
          blendConsts{blendConstants[0], blendConstants[1], blendConstants[2], blendConstants[3]},
          attachmentCount(colorBlending.size()), attachments(new ColorBlend[attachmentCount]) {
...
struct DescriptorSetLayoutBinding {
  public:
    ...
    DescriptorSetLayoutBinding() : binding(0), descriptorType(DescriptorType::UniformBuffer), stageFlags() {}

    DescriptorSetLayoutBinding(
        uint32_t binding, DescriptorType descriptorType, uint32_t descriptorCount, Flags<ShaderStage> stageFlags
    )
...
struct ImageSubresourceLayers {
    ...
    ImageSubresourceLayers(
        Flags<ImageAspect> aspectMask, uint32_t mipLevel = 0, uint32_t baseArrayLayer = 0, uint32_t layerCount = 1
    )
...
struct BufferCopyRegion {
    ...
    BufferCopyRegion(uint64_t size, uint64_t srcOffset = 0, uint64_t dstOffset = 0)
        : srcOffset(srcOffset), dstOffset(dstOffset), size(size) {}

    VULKAN_NATIVE_CAST_OPERATOR(BufferCopy);
};

template <typename T = uint32_t> struct Point2D {
    T x, y;
    Point2D() : x(0), y(0) {}
    template <typename OT> Point2D(OT scalar) : x(scalar), y(scalar) {}
    template <typename OT> Point2D(OT x, OT y) : x(x), y(y) {}

    VULKAN_NATIVE_CAST_OPERATOR(Offset2D);
};

template <typename T = uint32_t> struct Point3D {
    T x, y, z;
    Point3D() : x(0), y(0), z(0) {}
    template <typename OT> Point3D(OT scalar) : x(scalar), y(scalar), z(scalar) {}
    template <typename OT> Point3D(OT x, OT y, OT z) : x(x), y(y), z(z) {}
...
union ClearColor {
    ...
    /// @brief Create a clear structure for color attachments or images. Make sure that the type matches the format of
    /// the image.
    /// @param r Red
    /// @param g Green
    /// @param b Blue
...
struct ClearDepthStencil {
    ...
    ClearDepthStencil(float depthClearValue, uint32_t stencilClearValue)
...
struct ImageCopy {
    ...
    ImageCopy(
        ImageSubresourceLayers srcSubresource, uint32_t srcOffset, ImageSubresourceLayers dstSubresource,
        uint32_t dstOffset, uint32_t extent
    )
        : srcSubresource(srcSubresource), srcOffset{srcOffset, 0U, 0U}, dstSubresource(dstSubresource),
          dstOffset{dstOffset, 0U, 0U}, extent{extent, 1U, 1U} {}

    ImageCopy(
        ImageSubresourceLayers srcSubresource, Point2D<uint32_t> srcOffset, ImageSubresourceLayers dstSubresource,
        Point2D<uint32_t> dstOffset, Point2D<uint32_t> extent
    )
        : srcSubresource(srcSubresource), srcOffset{srcOffset.x, srcOffset.y, 0U}, dstSubresource(dstSubresource),
          dstOffset{dstOffset.x, dstOffset.y, 0U}, extent{extent.x, extent.y, 1U} {}

    ImageCopy(
        ImageSubresourceLayers srcSubresource, Point3D<uint32_t> srcOffset, ImageSubresourceLayers dstSubresource,
        Point3D<uint32_t> dstOffset, Point3D<uint32_t> extent
    )
        : srcSubresource(srcSubresource), srcOffset(srcOffset), dstSubresource(dstSubresource), dstOffset(dstOffset),
          extent(extent) {}
...
struct ImageBlit {
    ...
    ImageBlit()
        : srcOffsets{Point3D<int32_t>(), Point3D<int32_t>()}, dstOffsets{Point3D<int32_t>(), Point3D<int32_t>()} {}

    ImageBlit(
        ImageSubresourceLayers srcSubresource, const int32_t (&srcOffsets)[2], ImageSubresourceLayers dstSubresource,
        const int32_t (&dstOffsets)[2]
    )
        : srcSubresource(srcSubresource), srcOffsets{{srcOffsets[0], 0, 0}, {srcOffsets[1], 0, 0}},
          dstSubresource(dstSubresource), dstOffsets{{dstOffsets[0], 0, 0}, {dstOffsets[1], 0, 0}} {}

    ImageBlit(
        ImageSubresourceLayers srcSubresource, const Point2D<int32_t> (&srcOffsets)[2],
        ImageSubresourceLayers dstSubresource, const Point2D<int32_t> (&dstOffsets)[2]
    )
    ...
    ImageBlit(
        ImageSubresourceLayers srcSubresource, const Point3D<int32_t> (&srcOffsets)[2],
        ImageSubresourceLayers dstSubresource, const Point3D<int32_t> (&dstOffsets)[2]
    )
        : srcSubresource(srcSubresource), srcOffsets{srcOffsets[0], srcOffsets[1]}, dstSubresource(dstSubresource),
...
struct BufferImageCopy {
    ...
    BufferImageCopy(
        uint64_t bufferOffset, uint32_t bufferRowLength, uint32_t bufferImageHeight,
        ImageSubresourceLayers imageSubresource, uint32_t imageExtend, uint32_t imageOffset = 0U
    )
        : bufferOffset(bufferOffset), bufferRowLength(bufferRowLength), bufferImageHeight(bufferImageHeight),
          imageSubresource(imageSubresource), imageOffset{imageOffset, 0U, 0U}, imageExtend{imageExtend, 1U, 1U} {}

    BufferImageCopy(
        uint64_t bufferOffset, ImageSubresourceLayers imageSubresource, uint32_t imageExtend, uint32_t imageOffset = 0U
    )
        : bufferOffset(bufferOffset), bufferRowLength(0), bufferImageHeight(0), imageSubresource(imageSubresource),
          imageOffset{imageOffset, 0U, 0U}, imageExtend{imageExtend, 1U, 1U} {}

    BufferImageCopy(
        uint64_t bufferOffset, uint32_t bufferRowLength, uint32_t bufferImageHeight,
        ImageSubresourceLayers imageSubresource, Point2D<uint32_t> imageExtend,
        Point2D<uint32_t> imageOffset = Point2D<uint32_t>()
    )
        : bufferOffset(bufferOffset), bufferRowLength(bufferRowLength), bufferImageHeight(bufferImageHeight),
          imageSubresource(imageSubresource), imageOffset{imageOffset.x, imageOffset.y, 0U},
          imageExtend{imageExtend.x, imageExtend.y, 1U} {}

    BufferImageCopy(
        uint64_t bufferOffset, ImageSubresourceLayers imageSubresource, Point2D<uint32_t> imageExtend,
        Point2D<uint32_t> imageOffset = Point2D<uint32_t>()
    )
        : bufferOffset(bufferOffset), bufferRowLength(0), bufferImageHeight(0), imageSubresource(imageSubresource),
          imageOffset{imageOffset.x, imageOffset.y, 0U}, imageExtend{imageExtend.x, imageExtend.y, 1U} {}

    BufferImageCopy(
        uint64_t bufferOffset, uint32_t bufferRowLength, uint32_t bufferImageHeight,
        ImageSubresourceLayers imageSubresource, Point3D<uint32_t> imageExtend,
        Point3D<uint32_t> imageOffset = Point3D<uint32_t>()
    )
        : bufferOffset(bufferOffset), bufferRowLength(bufferRowLength), bufferImageHeight(bufferImageHeight),
          imageSubresource(imageSubresource), imageOffset(imageOffset), imageExtend(imageExtend) {}

    BufferImageCopy(
        uint64_t bufferOffset, ImageSubresourceLayers imageSubresource, Point3D<uint32_t> imageExtend,
        Point3D<uint32_t> imageOffset = Point3D<uint32_t>()
    )
        : bufferOffset(bufferOffset), bufferRowLength(0), bufferImageHeight(0), imageSubresource(imageSubresource),
          imageOffset(imageOffset), imageExtend(imageExtend) {}
...
struct DescriptorPoolSize {
    ...
    DescriptorPoolSize(DescriptorType descriptorType, uint32_t descriptorCount)
        : descriptorType(descriptorType), descriptorCount(descriptorCount) {}
...
struct ImageSubresource {
    ...
    ImageSubresource(
        ImageAspect aspectMask, uint32_t baseMipLevel = 0, uint32_t levelCount = 1, uint32_t baseArrayLayer = 0,
        uint32_t layerCount = 1
    )
        : aspectMask(aspectMask), baseMipLevel(baseMipLevel), levelCount(levelCount), baseArrayLayer(baseArrayLayer),
...
struct BufferMemoryBarrier {
  ...
  public:
    ...
    BufferMemoryBarrier() : srcQueueFamilyIndex(0), dstQueueFamilyIndex(0), offset(0), size(0) {}

    BufferMemoryBarrier(
        Flags<Access> srcAccessMask, Flags<Access> dstAccessMask, const Queue &srcQueue, const Queue &dstQueue,
        BufferHandle buffer, uint64_t offset, uint64_t size
    )
        : srcAccessMask(srcAccessMask), dstAccessMask(dstAccessMask), srcQueueFamilyIndex(srcQueue.GetIndex()),
          dstQueueFamilyIndex(dstQueue.GetIndex()), buffer(buffer), offset(offset), size(size) {}
    BufferMemoryBarrier(
        Flags<Access> srcAccessMask, Flags<Access> dstAccessMask, const Queue &dstQueue, BufferHandle buffer,
        uint64_t offset, uint64_t size
    )
        : srcAccessMask(srcAccessMask), dstAccessMask(dstAccessMask), srcQueueFamilyIndex(~0U),
          dstQueueFamilyIndex(dstQueue.GetIndex()), buffer(buffer), offset(offset), size(size) {}
    BufferMemoryBarrier(
        Flags<Access> srcAccessMask, Flags<Access> dstAccessMask, BufferHandle buffer, uint64_t offset, uint64_t size
    )
        : srcAccessMask(srcAccessMask), dstAccessMask(dstAccessMask), srcQueueFamilyIndex(~0U),
          dstQueueFamilyIndex(~0U), buffer(buffer), offset(offset), size(size) {}
...
struct ImageMemoryBarrier {
  ...
  public:
    ...
    ImageMemoryBarrier()
        : oldLayout(ImageLayout::Undefined), newLayout(ImageLayout::Undefined), srcQueueFamilyIndex(0),
          dstQueueFamilyIndex(0) {}

    ImageMemoryBarrier(
        ImageHandle image, ImageLayout oldLayout, ImageLayout newLayout, Flags<Access> srcAccessMask,
        Flags<Access> dstAccessMask, const Queue &srcQueue, const Queue &dstQueue, ImageSubresource subresourceRange
    )
        : srcAccessMask(srcAccessMask), dstAccessMask(dstAccessMask), oldLayout(oldLayout), newLayout(newLayout),
          srcQueueFamilyIndex(srcQueue.GetIndex()), dstQueueFamilyIndex(dstQueue.GetIndex()), image(image),
          subresourceRange(subresourceRange) {}
    ImageMemoryBarrier(
        ImageHandle image, ImageLayout oldLayout, ImageLayout newLayout, Flags<Access> srcAccessMask,
        Flags<Access> dstAccessMask, const Queue &dstQueue, ImageSubresource subresourceRange
    )
        : srcAccessMask(srcAccessMask), dstAccessMask(dstAccessMask), oldLayout(oldLayout), newLayout(newLayout),
          srcQueueFamilyIndex(~0U), dstQueueFamilyIndex(dstQueue.GetIndex()), image(image),
          subresourceRange(subresourceRange) {}
    ImageMemoryBarrier(
        ImageHandle image, ImageLayout oldLayout, ImageLayout newLayout, Flags<Access> srcAccessMask,
        Flags<Access> dstAccessMask, ImageSubresource subresourceRange
    )
        : srcAccessMask(srcAccessMask), dstAccessMask(dstAccessMask), oldLayout(oldLayout), newLayout(newLayout),
          srcQueueFamilyIndex(~0U), dstQueueFamilyIndex(~0U), image(image), subresourceRange(subresourceRange) {}
    ImageMemoryBarrier(
        ImageHandle image, ImageLayout newLayout, Flags<Access> srcAccessMask, Flags<Access> dstAccessMask,
        const Queue &srcQueue, const Queue &dstQueue, ImageSubresource subresourceRange
    )
        : srcAccessMask(srcAccessMask), dstAccessMask(dstAccessMask), oldLayout(ImageLayout::Undefined),
          newLayout(newLayout), srcQueueFamilyIndex(srcQueue.GetIndex()), dstQueueFamilyIndex(dstQueue.GetIndex()),
          image(image), subresourceRange(subresourceRange) {}
    ImageMemoryBarrier(
        ImageHandle image, ImageLayout newLayout, Flags<Access> srcAccessMask, Flags<Access> dstAccessMask,
        const Queue &dstQueue, ImageSubresource subresourceRange
    )
        : srcAccessMask(srcAccessMask), dstAccessMask(dstAccessMask), oldLayout(ImageLayout::Undefined),
          newLayout(newLayout), srcQueueFamilyIndex(~0U), dstQueueFamilyIndex(dstQueue.GetIndex()), image(image),
          subresourceRange(subresourceRange) {}
    ImageMemoryBarrier(
        ImageHandle image, ImageLayout newLayout, Flags<Access> srcAccessMask, Flags<Access> dstAccessMask,
        ImageSubresource subresourceRange
        ...
        : srcAccessMask(srcAccessMask), dstAccessMask(dstAccessMask), oldLayout(ImageLayout::Undefined),
          newLayout(newLayout), srcQueueFamilyIndex(~0U), dstQueueFamilyIndex(~0U), image(image),
          subresourceRange(subresourceRange) {}
...
struct MemoryBarrier {
  ...
  public:
    ...
    MemoryBarrier() {}
    MemoryBarrier(Flags<Access> srcAccessMask, Flags<Access> dstAccessMask)
...
struct SubmitInfo {
  ...
  public:
    ...
    SubmitInfo(
        const std::vector<std::tuple<Flags<PipelineStage>, SemaphoreHandle>> &waitStages = {},
        const std::vector<SemaphoreHandle> &signalSemaphores = {}, const std::vector<CmdBufferHandle> &cmdBuffers = {}
    )
        : waitSemaphoreCount(waitStages.size()), waitSemaphores(new SemaphoreHandle[waitStages.size()]),
          waitDstStageMask(new Flags<PipelineStage>[waitStages.size()]), cmdBufferCount(cmdBuffers.size()),
          cmdBuffers(new CmdBufferHandle[cmdBuffers.size()]), signalSemaphoreCount(signalSemaphores.size()),
          signalSemaphores(new SemaphoreHandle[signalSemaphores.size()]) {
    ...
    SubmitInfo(SubmitInfo &&rhs) noexcept : SubmitInfo() { *this = std::move(rhs); }
...
struct PushConstantRange {
    ...
    PushConstantRange(Flags<ShaderStage> stageFlags = {}, uint32_t offset = 0, uint32_t size = 0)
